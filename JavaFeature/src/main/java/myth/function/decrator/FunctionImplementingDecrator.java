package myth.function.decrator;
...
public class FunctionImplementingDecrator {
  ...
  public static void main(String[] args) {
    ...
    Function<Double, Double> sumSquare = op(pow(Math::sin, 2), pow(Math::cos, 2))
        .apply((a, b) -> a + b);
    System.out.println(sumSquare.apply(x));

    Function<Double, Double> another = op(compose((d) -> d * d, Math::sin),
        compose((d) -> d * d, Math::cos)).apply((a, b) -> a + b);
    System.out.println(another.apply(x));

    Function<Double, Double> third = compose(d -> d * d, d -> d + 1, d -> d * 2,
        d -> d * d * d);  // (2x^3+1)^2
   ...
   */
  public static <T> Function<BiFunction<T, T, T>, T> op(Function<T, T> funcx, Function<T, T> funcy,
      T x) {
  ...
  public static <T> Function<T, T> compose(Function<T, T>... extraFuncs) {
    ...
    return x -> Arrays.stream(extraFuncs).reduce(y -> y, FunctionImplementingDecrator::compose)
        .apply(x);
  }

  public static <T> Function<BiFunction<T, T, T>, Function<T, T>> op(Function<T, T> funcx,
      Function<T, T> funcy) {
