...
class AutoUTGenerator {
    ...
    static buildUTContent(AllInfoForAutoGeneratingUT allInfo) {
        def spockTestFile = new File("${projectRoot}/resources/spocktest.tpl")
    ...
    static generateTestMethod(MethodInfo m, String className) {
        def engine = new SimpleTemplateEngine()
        def methodTplFile = new File("${projectRoot}/resources/method.tpl")
    ...
    static class AllInfoForAutoGeneratingUT {
        String packageName
        String className
