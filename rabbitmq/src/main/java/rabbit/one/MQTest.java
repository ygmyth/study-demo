...
import static rabbit.one.RabbitMqConfig.TOPIC_QUEUE_ONE;
...
//            channel.basicNack(message.getMessageProperties().getDeliveryTag(), false, false);
  ...
  public void sendTopic() {
    ...
    log.info("订阅者模式->发送消息:routing_key");
    rabbitTemplate.convertSendAndReceive("topic_exchange", "routing_key.one",
        "routing_key.one " + LocalTime.now(), cId);
    log.info("订阅者模式->发送消息routing_key_two");
    rabbitTemplate.convertSendAndReceive("topic_exchange", "routing_key.two",
        "routing_key.two" + LocalTime.now(), cId01);
  }
  ...
  public void topic1(String msg, Channel channel, Message message) throws IOException {
    long deliveryTag = message.getMessageProperties().getDeliveryTag();
    try {
      log.info("接收到topic_routing_key消息:{}", msg);
      channel.basicAck(deliveryTag, false);
    } catch (Exception e) {
